...
This vignette will give you an example of how scRNA-seq data can be processed using the [`Seurat toolkit`](https://satijalab.org/seurat/) and then exported for visualization in Cerebro.
...
```{r installation}
if ( 'cerebroApp' %in% installed.packages() == FALSE ) BiocManager::install('cerebroApp')
if ( 'Seurat' %in% installed.packages() == FALSE ) BiocManager::install('Seurat')
if ( 'monocle' %in% installed.packages() == FALSE ) BiocManager::install('monocle')
```
...
Using the functions provided by `cerebroApp`, we check the percentage of mitochondrial and ribosomal genes and, for every sample (even though we have only one) and cluster, we...
