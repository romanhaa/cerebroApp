...
output[["trajectory_UI"]] <- renderUI({
  if ( length(sample_data()$trajectories$monocle2) > 0 ) {
    tagList(
      fluidRow(
...
output[["trajectory_input"]] <- renderUI({
  tagList(
    selectInput(
      ...
      label = "Trajectory",
      choices = names(sample_data()$trajectories$monocle2)
    ),
...
output[["trajectory_projection"]] <- plotly::renderPlotly({
  ...
  to_plot <- cbind(
      sample_data()$trajectories$monocle2[[ trajectory_to_display ]][["meta"]][ cells_to_display , ],
  ...
  trajectory_edges <- sample_data()$trajectories$monocle2[[trajectory_to_display]][["edges"]]
  ...
  if ( exists("plot_export_path") ) {
    ...
    if ( is.factor(to_plot[[ color_variable ]]) || is.character(to_plot[[ color_variable ]]) ) {
      ...
      p <- ggplot() +
        ...
        geom_segment(
          data = sample_data()$trajectories$monocle2[[ trajectory_to_display ]]$edges,
...
output[["trajectory_density_plot"]] <- plotly::renderPlotly({
  ...
  if ( is.factor(to_plot[[ color_variable ]]) || is.character(to_plot[[ color_variable ]]) ) {
    ...
    colors_this_plot <- setNames(
      default_colorset[1:length(levels(sample_data()$trajectories$monocle2[[ input[["trajectory_to_display"]] ]][["meta"]]$state))],
      levels(sample_data()$trajectories$monocle2[[ input[["trajectory_to_display"]] ]][["meta"]]$state)
    )
    plot <- plotly::plot_ly(
      data = to_plot,
      x = ~pseudotime,
...
output[["trajectory_number_of_cells_by_state_table"]] <- DT::renderDataTable(server = FALSE, {
  ...
  if ( is.numeric(sample_data()$cells[[ input[["trajectory_dot_color"]] ]]) || input[["trajectory_dot_color"]] == "state" || input[["trajectory_dot_color"]] == "pseudotime" ) {
    table <- sample_data()$trajectories$monocle2[[ input[["trajectory_to_display"]] ]][["meta"]] %>%
      dplyr::filter(!is.na(pseudotime)) %>%
      dplyr::group_by(state) %>%
      dplyr::summarize(total_cell_count = n())
  } else {
    table <- cbind(
        sample_data()$trajectories$monocle2[[ input[["trajectory_to_display"]] ]][["meta"]],
